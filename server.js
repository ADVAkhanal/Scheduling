...
function parseLoad(wb, months) {
  ...
  data.forEach(r => {
    ...
    wos.push({
      ...
      must_leave: dt.toISOString().slice(0, 10), cust_due: dueStr || null, status: st,
...
app.post('/api/upload/capacity', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    for (const wc of wcData) {
      ...
      for (let i = 0; i < months.length; i++) {
        await client.query(
          `INSERT INTO mps_wc_months (wc_id, dataset_id, month_idx, cap, load) VALUES ($1,$2,$3,$4,0)
  ...
  try {
    ...
    for (const wc of wcsRes.rows) {
      for (let i = 0; i < monthLabels.length; i++) {
        ...
        await client.query('UPDATE mps_wc_months SET load=$1 WHERE wc_id=$2 AND month_idx=$3', [load, wc.id, i]);
      }
    }
